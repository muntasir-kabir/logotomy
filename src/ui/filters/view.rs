...
use crate::ui::app::model::{Filter, LogTab, MAX_FILTERS};
    ...
    egui::Frame::default()
        ...
        .show(ui, |ui| {
            ui.horizontal(|ui| {
                ...
                let resp = ui.add_enabled_ui(!at_cap, |ui| {
                    ui.add_sized(
                        egui::vec2(225.0, 0.0),
                        egui::TextEdit::singleline(&mut tab.filter_input)
                            .hint_text(if at_cap { "Max 20 filters" } else { "filter + Enter" })
                            .desired_width(225.0),
                    )
                }).inner;
                let enter = resp.lost_focus() && ui.input(|i| i.key_pressed(egui::Key::Enter));
                let add_clicked = ui.add_enabled(
                    !at_cap,
                ...
                if (enter || add_clicked) && !tab.filter_input.trim().is_empty() {
                    ...
                    if !tab.filters.iter().any(|k| k.text == text) {
                        let color = theme.filter_colors[tab.filters.len() % theme.filter_colors.len()];
                        tab.filters.push(Filter { text, color });
                        changed = true;
                    }
